...
class ExternalApi {
    ...
    /**
     * Send the plugin status to the Csqita server
    ...
    static function update_plugins_status( $status = 'install' ) {
        ...
        $token      = get_option( 'csqita_token', '' );
        $user_id    = get_option( 'csqita_user_identifier', '' );
     ...
     * @return void
     */
    static function sync_csqita_sercet_key() {
        $secret_key = get_option( 'csqita_api_secret_license_key', '' );
        if(empty($secret_key)) {
            $token      = get_option( 'csqita_token', '' );
            if(empty($token)) {
                return;
            ...
            if(!is_wp_error($response) && 200 === wp_remote_retrieve_response_code( $response )) {
                ...
                if(isset($response['message']) && $response['message'] == 'Success') {
                    add_option( 'csqita_api_secret_license_key', $secret_key );
                }
            }
        }
    }
    ...
    /**
     * Retrieves the secret key for Csqita.
     *
     ...
     */
    static function get_csqita_secret_key() {
        $secret_key    = get_option( 'csqita_secret_key', '' );
        if(!empty($secret_key)) {
            return $secret_key;
        }
        ...
        if (isset($response_code['secret_key'])) {
            ...
            add_option( 'csqita_secret_key', $response_code['secret_key'] );
     ...
     */
    public static function sync_wp_plugin_version($status = '', $csqita_status = 1) {
        $has_error = get_option('csqita_has_auth_error', '');
        if($has_error == 'yes') {
            return;
        }
        ...
        if (!empty($token) && !empty($user_id)) {
            ...
            $request = [
                ...
                'body'     => [
                    'site_url'  => site_url(),
                    'version'  => \Csqita::version(),
                    'is_woocommerce_installed' => $status,
                    'is_active_install' => $csqita_status
                ]
            ];
            ...
            if(isset($response_code['message']) && $response_code['message'] == 'Success') {
                update_option('csqita_wp_plugin_version', \Csqita::version());
            }

            self::sync_csqita_sercet_key();
        }
    }

    static function csqita_logout()
    {
        $token      = get_option('csqita_token', '');
        if(empty($token)) {
            return;
