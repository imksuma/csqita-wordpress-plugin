(function () {
  ...
  function LoginRequest(username, password) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        const res = await fetch(window.csqita.remoteEndpoint+'/anon/v1/login', {
          method: 'POST',
          body: JSON.stringify({username: username.value, password: password.value, site: window.csqita.siteUrl}),
          headers: {
  ...
  function ActivateCardRequest(cardId) {
    ...
    return new Promise(async function (resolve, reject) {
      const res = await fetch(window.csqita.remoteEndpoint+'/user/v1/chatbot', {
        method: 'POST',
        body: JSON.stringify({id: cardId, domain: window.csqita.domain}),
        headers: {
      ...
      if (res.status === 200) {
        const res = await fetch(window.csqita.remoteEndpoint+'/user/v1/integration/token', {
          method: 'POST',
    ...
    return new Promise(async function (resolve, reject) {
      ...
      if (res.status === 200) {
        ...
        for (let index = 0; index < chatbots.length; index++) {
          ...
          const res = await fetch(window.csqita.remoteEndpoint+`/user/v1/chatbot?id=${element.id}`, {
            method: 'GET',
            headers: {
              Authorization: 'Bearer ' + getToken(),
              'Content-Type': 'application/json',
          ...
          if (check['chatbot'] !== null) {check['chatbot'] = element}
          if (res.status === 200) {
            const data = await res.json();
            element['domain'] = data?.data;
            element['status'] = (element['domain'] === window.csqita.siteUrl) ? 'active' : 'deactive';
            check.isActive = check.isActive || (element['domain'] === window.csqita.siteUrl);
            element['available'] = (element['domain'] === '') || (element['domain'] === window.csqita.siteUrl);
          ...
          if (!check.isActive) {
            ...
            var chatbot = chatbotsD.find(function (c) { return c.id === check['chatbot'].id; });
  ...
  function renderDashboard() {
    ...
    document.getElementById('csqita-logout-btn').addEventListener('click', function () {
      wp.apiFetch({
  ...
  function renderCardGrid(cards) {
    ...
    grid.innerHTML = cardss.map(function (card) {
      var isActive = card.status === 'active';
      return (
        '<div class="csqita-card" data-status="' + card.status + '" data-card-id="' + card.id + '">' +
          '<div class="csqita-card-name">' + escapeHtml(card.name) + '</div>' +
          (card.available) ? ('<span class="csqita-status" data-status="' + card.status + '">' +
          ...
          (isActive ? ''
            ...
            '<div class="csqita-card-footer">' +
              '<button class="csqita-activate-btn" data-card-id="' + card.id + '">' +
                '<span class="csqita-spinner"></span>' +
                '<span class="csqita-activate-label">Activate</span>' +
              '</button>' +
            '</div>'
          )) : ('<span class="csqita-status" data-status="' + card.status + '">' +
            '<span class="csqita-status-dot"></span>' +
              'used for ' + card.domain + ', please deactive it from ' + card.domain + ' before use it for this domain. you can deactive it from csqita.com by fill the domain as blank.' +
            '</span>'
          ) +
        '</div>'
      );
  ...
  function handleActivate(cardId) {
    var btn = document.querySelector('.csqita-activate-btn[data-card-id="' + cardId + '"]');
    var errorBox = document.getElementById('csqita-dashboard-error');
